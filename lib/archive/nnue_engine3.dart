...
class SearchTables {
  final List<List<Move?>> killers = List.generate(128, (_) => [null, null]);

  final List<List<int>> history = List.generate(128, (_) => List.filled(64, 0));
  ...
  void addHistory(Color side, Move mv, int depth) {
    if (!squareInBoard(mv.from) || !squareInBoard(mv.to)) return;
    int s = side == Color.WHITE ? 0 : 1;
    int idx = s * 64 + mv.from;
...
// ================================================================
// NEGAMAX (PVS + TT + LMR + FUTILITY + SAFE WINDOW CHECK)
// ================================================================
...
double negamax(
  ...
  for (final mv in moves) {
    ...
    if (depth == 1 && !pos.in_check && isQuiet(mv)) {
      double eval = color * pos.nnue.evaluateBoard(pos.board, pos.turn);
...
// ================================================================
// ROOT SEARCH WITH SAFE ASPIRATION WINDOWS
// ================================================================
...
RootMove rootSearch(
  ...
  // Safe aspiration
  if (prevEval != null) {
    double w = 300; // NNUE evals can vary widely
    alpha = prevEval - w;
    beta = prevEval + w;
