...
// incremental updates, and full training (MSE + Adam) with stable gradients.
//
// Fixes to enable learning:
//  - Smaller L0 init std to avoid early C0 saturation.
//  - Backprop uses pre-activation and a *leaky* ClippedReLU derivative.
...
//   - constants.dart         : BITS_EP_CAPTURE, BITS_KSIDE_CASTLE, BITS_QSIDE_CASTLE, BITS_PROMOTION
//   - nnue_logic_batch2.dart : TrainingPosition(board: List<Piece?>, turn: Color, target: double)
...
class NNUERef {
  ...
  NNUERef({int seed = 42}) {
    ...
    // Use MUCH smaller std for L0 to avoid saturation at C0.
    l0 = _Linear(NUM_FEATURES, M)..gaussianInit(rng, std: 0.02);
    // Keep a modest He-like init for L1/L2.
    l1 = _Linear(2 * M, K)..heInit(rng, gain: 1.1);
    l2 = _Linear(K, 1)..heInit(rng, gain: 1.1);
  ...
  /// Deep clone weights/bias from another NNUERef (moments reset).
  static NNUERef cloneFrom(NNUERef src) {
    final dst = NNUERef();
    // l0
    for (int c = 0; c < NUM_FEATURES; c++) {
      final sw = src.l0.weights[c];
      final dw = dst.l0.weights[c];
      for (int r = 0; r < M; r++) {
        dw[r] = sw[r];
      }
    }
    for (int r = 0; r < M; r++) {
      dst.l0.bias[r] = src.l0.bias[r];
    }
    // l1
    for (int c = 0; c < 2 * M; c++) {
      final sw = src.l1.weights[c];
      final dw = dst.l1.weights[c];
      for (int r = 0; r < K; r++) {
        dw[r] = sw[r];
      }
    }
    for (int r = 0; r < K; r++) {
      dst.l1.bias[r] = src.l1.bias[r];
    }
    // l2
    for (int r = 0; r < K; r++) {
      dst.l2.weights[r][0] = src.l2.weights[r][0];
    }
  ...
  /// Leaky derivative for ClippedReLU using *pre-activation* value.
  ///  - if 0 < x < 1 : 1.0
  ///  - else         : leakSlope (e.g., 0.01)
  @pragma('vm:prefer-inline')
  static double dCreluLeaky(double pre, {double leakSlope = 0.01}) {
    return (pre > 0.0 && pre < 1.0) ? 1.0 : leakSlope;
  }
  ...
  // Convenience: eval with fresh accumulator
  double evalBoardCp(List<Piece?> board0x88, Color stm) {
    final acc = newAccumulator();
    refreshAccumulator(acc, board0x88);
    return evaluate(acc, stm);
  }
  ...
  void trainBatch(List<TrainingPosition> batch, double lr) {
    ...
    double _clip(double g, [double c = 1.0]) {
      if (g > c) return c;
      if (g < -c) return -c;
      return g;
    }
    ...
    for (final pos in batch) {
      ...
      final inL1Pre = Float64List(2 * M);
      inL1Pre.setRange(0, M, acc.v[stm]);
      inL1Pre.setRange(M, 2 * M, acc.v[oth]);

      final inL1 = Float64List.fromList(inL1Pre);
      for (int i = 0; i < 2 * M; i++) {
        inL1[i] = crelu(inL1[i]);
      }

      final out1Pre = l1.forwardDense(inL1);
      final out1 = Float64List.fromList(out1Pre);
      for (int i = 0; i < K; i++) {
        out1[i] = crelu(out1[i]);
      }
      ...
      for (int j = 0; j < K; j++) {
        final dact = dCreluLeaky(out1Pre[j], leakSlope: 0.01);
        final back = gradOut * l2.weights[j][0] * dact;
        gL2Out[j] = back;
        gL2W[j] += _clip(gradOut * out1[j]);
      }
      gL2B += _clip(gradOut);
      ...
      for (int i = 0; i < 2 * M; i++) {
        ...
        for (int j = 0; j < K; j++) {
          ...
          gL1W[base + j] += _clip(gL2Out[j] * inL1[i]);
        }
        final dact0 = dCreluLeaky(inL1Pre[i], leakSlope: 0.01);
        gL1Out[i] = sum * dact0;
      }
      for (int j = 0; j < K; j++) {
        gL1B[j] += _clip(gL2Out[j]);
      }

      // ----- L0 bias grads -----
      for (int j = 0; j < M; j++) {
        gL0B[j] += _clip(gL1Out[j] + gL1Out[M + j]);
      }
      ...
      for (final f in wf) {
        ...
        for (int j = 0; j < M; j++) {
          row[j] += _clip(gL1Out[wOffset + j]);
        }
      }
      for (final f in bf) {
        final row = gL0W.putIfAbsent(f, () => Float64List(M));
        for (int j = 0; j < M; j++) {
          row[j] += _clip(gL1Out[bOffset + j]);
        }
      }
    }
    ...
    gL0W.forEach((idx, grads) {
      ...
      for (int j = 0; j < M; j++) {
        _m0[idx]![j] = b1 * mRow[j] + (1 - b1) * grads[j];
        _v0[idx]![j] = b2 * vRow[j] + (1 - b2) * (grads[j] * grads[j]);
        wRow[j] -=
            lr * ((_m0[idx]![j] / bc1) / (sqrt(_v0[idx]![j] / bc2) + 1e-8));
      }
    });
...
class _Linear {
  ...
  /// Gaussian init with std; biases zero.
  void gaussianInit(Random rng, {double std = 0.02}) {
    for (int c = 0; c < inSize; c++) {
      final col = weights[c];
      for (int r = 0; r < outSize; r++) {
        // Box-Muller
        final u1 = max(rng.nextDouble(), 1e-9);
        final u2 = rng.nextDouble();
        final z = sqrt(-2.0 * log(u1)) * cos(2 * pi * u2); // ~N(0,1)
        col[r] = z * std;
      }
    }
    for (int r = 0; r < outSize; r++) {
  ...
  /// He-like init with a gain.
  void heInit(Random rng, {double gain = 1.0}) {
    final s = sqrt(2.0 / inSize) * gain;
    for (int c = 0; c < inSize; c++) {
      final col = weights[c];
      for (int r = 0; r < outSize; r++) {
